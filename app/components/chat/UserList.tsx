	...
	return (
		<div className="flex flex-col h-full">
			<div className="flex items-center justify-between px-5 py-4 border-b border-(--border)">
				<p className="text-lg font-bold text-(--text-primary) tracking-tight shrink-0">
					{tr.chats}
				</p>

				<div className="relative flex items-center w-10/13">
					<Search
						className="absolute left-3 text-(--text-primary)/30 pointer-events-none"
						size={15}
						strokeWidth={1.8}
					/>
					<input
			...
			{/* Папки */}
			<div className="p-3 flex items-center gap-1 overflow-x-auto [scrollbar-width:none] border-b border-(--border)">
				<button
				...
				<button
					...
					className={`shrink-0 px-3 py-1 rounded-lg text-xs font-medium transition-colors ${activeFolder === 'unread' ? 'bg-(--bg-card) text-(--text-primary)' : 'text-(--text-primary)/40 hover:text-(--text-primary)'}`}
				>
					{tr.unread}
					{totalUnread > 0 && (
